...
- `Bot.restart()` 可从消息处理器或后台任务调用。宿主先取得重启任务锁，再通过 IPC 确认请求；忙碌时调用会失败，不会静默丢弃。确认只表示请求已接收，新 Worker 的启动结果由宿主日志报告。
- 重启会终止当前 Worker。插件需要发送的通知应在调用 `Bot.restart()` 前完成并等待回复结果；确认后的异步工作不保证完成。
- `e.group.getMsg(message_id)` 返回转换后的 icqq 风格消息段，保留消息 ID、发送者等信息，补齐 `user_id`、`seq` 和 `toString()`。查询失败会拒绝 Promise，插件可自行捕获并降级处理。
