...
/** GitHub 代理前缀 */
function getGhProxy() {
	return getConfig$1()?.gh_proxy ?? DEFAULT_GH_PROXY;
}
/** 默认 Yunzai 仓库地址 */
...
function getPluginAliasMap() {
	...
	for (const [alias, raw] of Object.entries(custom)) if (raw && typeof raw === "object" && raw.dirName && raw.repoUrl) {
		const info = {
			dirName: raw.dirName,
			repoUrl: raw.repoUrl,
...
function getAllPlugins(variant) {
	const result = BUILTIN_PLUGINS.filter((plugin) => !variant || !plugin.variants || plugin.variants.includes(variant));
