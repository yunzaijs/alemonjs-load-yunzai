...
import { backupYunzaiData, deleteLogFile, deletePluginArchive, extractPluginArchive, getDataBackupList, getLogViewerData, getPluginArchiveData, getRepoData, getRepositoryArchiveData, getStatusData, getYunzaiFormData, repairRepositoryArchiveSource, restoreYunzaiDataBackup, runYunzaiAction, saveRepoData, saveYunzaiFormData, unpackRepositoryArchive, uploadDataBackup, uploadPluginArchive, uploadRepositoryArchive, uploadYunzaiPluginArchive } from "./panel-service.js";
...
const PLUGIN_UPLOAD_MAX_BYTES = 2147483648;
const UPLOAD_FILE_TTL_MS = 216e5;
...
function cleanupUploadDir() {
	...
	for (const file of files) if (now - file.createdAt > UPLOAD_FILE_TTL_MS) try {
...
apiRouter.post("/repo/archive-extract", async (ctx) => {
	try {
		...
		const data = await unpackRepositoryArchive(target);
		ctx.status = 200;
		ctx.body = {
			code: 200,
			message: "压缩包解压完成",
...
apiRouter.post("/repo/archive-repair-origin", async (ctx) => {
	try {
		...
		const data = await repairRepositoryArchiveSource(body.target, String(body.repoUrl ?? ""));
		ctx.status = 200;
		ctx.body = {
			code: 200,
			message: "仓库来源已修复",
...
apiRouter.post("/yunzai/plugin-upload", async (ctx) => {
	...
	const rawDirName = ctx.request.body?.dirName;
...
apiRouter.post("/repo/archive-upload", async (ctx) => {
	...
	try {
		const data = uploadRepositoryArchive(target, uploadedFile.path, uploadedFile.originalname);
		ctx.status = 200;
		ctx.body = {
			code: 200,
			message: "压缩包已保存；重新上传会覆盖当前压缩包",
			data
		};
	} catch (err) {
		ctx.status = 400;
