import { deleteLogFile, getLogViewerData, getStatusData } from "./panel-service.js";
...
const activate = (context) => {
	...
	webView.onMessage(async (data) => {
		try {
			...
			else if (data.type === "yunzai.logs.delete") webView.postMessage({
			...
			console.error(e);
			if (data.type === "yunzai.logs.delete") webView.postMessage({
				type: "yunzai.result",
				data: { message: e instanceof Error ? e.message : "日志删除失败" }
			});
		}
	});
};
