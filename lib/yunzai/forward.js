...
function withoutType(value) {
	return Object.fromEntries(Object.entries(value).filter(([key]) => key !== "type"));
}
function withDefinedValues(value, keys) {
	return Object.fromEntries(keys.filter((key) => value[key] !== void 0 && value[key] !== null).map((key) => [key, value[key]]));
}
/**
...
* 在调用点猜字段。
*/
function toOneBotSegment(value) {
	...
	if (hasOneBotData && type !== "node") {
		...
		if ([
			...
			"video",
			"flash"
		].includes(type) && data.file !== void 0) data.file = normalizeOneBotMediaSource(data.file);
		if (type === "json" && typeof data.data !== "string") data.data = JSON.stringify(data.data ?? "");
		if (type === "poke") {
		...
		if (type === "music" && data.type === void 0 && data.platform !== void 0) {
	...
	switch (type) {
		case "text": return {
			type: "text",
			data: { text: String(source.text ?? "") }
		};
		case "at": return {
			type: "at",
			data: { qq: String(source.qq ?? source.id ?? "") }
		};
		case "face":
		case "sface": return {
			type,
			data: withDefinedValues(source, ["id", "text"])
		};
		case "image":
		case "flash":
		case "record":
		case "video": {
			const data = withDefinedValues(source, [
				"cache",
				"proxy",
				"timeout",
				"type",
			...
			if (source.file !== void 0) data.file = normalizeOneBotMediaSource(source.file);
			if (type === "flash") {
				data.type ??= "flash";
				return {
					type: "image",
		...
		case "json": return {
			type: "json",
			data: { data: typeof source.data === "string" ? source.data : JSON.stringify(source.data ?? "") }
		};
		case "xml": return {
			type: "xml",
		...
		case "forward": return {
			type,
			data: withDefinedValues(source, ["id"])
		};
		case "rps":
		case "dice":
		case "shake": return {
		...
		case "poke": return {
			type: "poke",
			data: {
				type: String(source.poke_type ?? source.pokeType ?? 1),
				id: String(source.id ?? "")
			}
		};
		case "share": return {
			type: "share",
			data: withDefinedValues(source, [
				"url",
				"title",
				"content",
				"image"
		...
		case "location": return {
			...
			data: withDefinedValues({
				lat: source.lat,
				lon: source.lon ?? source.lng,
				title: source.title ?? source.name,
				content: source.content ?? source.address
			}, [
				"lat",
				"lon",
				"title",
		...
		case "node": return toOneBotForwardNode(value);
		default: return {
			type,
			data: source
		};
...
function toOneBotMessage(value) {
	if (typeof value === "string") return value;
	return (Array.isArray(value) ? value : [value]).map(toOneBotSegment);
}
/**
	...
	for (const content of contents) {
		...
		switch (content.type) {
			...
			case "xml":
				...
				break;
			case "raw":
				if (content.nativeType && content.nativeData) result.push(toOneBotSegment({
