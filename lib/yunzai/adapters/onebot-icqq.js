...
function createOneBotRuntime(deps) {
    ...
    function fetchMemberRecord(groupId, userId, noCache = false) {
        ...
        return callApi('getGroupMemberInfo', { group_id: groupId, user_id: userId, ...(noCache ? { no_cache: true } : {}) })
            .then((res) => cacheMemberRecord(groupId, userId, res?.data ?? cached))
            .catch(() => cached);
    }
    ...
    function createOneBotGroupAdapter(groupId, opts) {
        return wrapCompatValue({
            ...
            pickMember: (uid) => {
                ...
                const updateAfter = (action, params, patch = {}, remove = false) => callApi(action, params)
                    .then((result) => {
                    if (remove) {
                ...
                return wrapCompatValue({
                    ...
                    mute: (duration = 600) => updateAfter('setGroupBan', { group_id: groupId, user_id: uid, duration }, { shut_up_timestamp: duration > 0 ? Math.floor(Date.now() / 1000) + duration : 0 }),
            ...
            setName: (name) => callApi('setGroupName', { group_id: groupId, group_name: name })
                .then((result) => {
            ...
            getInfo: () => callApi('getGroupInfo', { group_id: groupId })
                .then((res) => cacheGroupRecord(groupId, res?.data ?? {}, opts))
            ...
            muteAnony: (flag, duration = 1800) => callApi('set_group_anonymous_ban', { group_id: groupId, anonymous_flag: flag, duration }).catch(() => { }),
            ...
            sendFile: (file, _pid, name) => callApi('upload_group_file', { group_id: groupId, file: String(file), name: name ?? 'file' }).catch(() => ({})),
            ...
            setScreenMemberMsg: (memberId, isScreen = true) => callApi('_set_group_screen_msg', { group_id: groupId, user_id: memberId, is_screen: isScreen }).catch(() => false),
            ...
            fs: wrapCompatValue({
                ...
                ls: (pid = '/', start = 0, limit = 100) => callApi('get_group_files_by_folder', { group_id: groupId, folder_id: pid, start, limit })
                    .then((res) => [...(res?.data?.files ?? []), ...(res?.data?.folders ?? [])])
                    .catch(() => []),
                mkdir: (name) => callApi('create_group_file_folder', { group_id: groupId, name, parent_id: '/' })
                    .then((res) => res?.data ?? {})
                    .catch(() => ({})),
                rm: (fid) => callApi('delete_group_file', { group_id: groupId, file_id: fid }).catch(() => { }),
                rename: (fid, name) => callApi('_rename_group_file', { group_id: groupId, file_id: fid, name }).catch(() => { }),
                mv: (fid, pid) => callApi('_move_group_file', { group_id: groupId, file_id: fid, parent_id: pid }).catch(() => { }),
                upload: (file, pid = '/', name) => callApi('upload_group_file', { group_id: groupId, file: String(file), name: name ?? 'file', folder: pid })
                    .then((res) => res?.data ?? {})
                    .catch(() => ({})),
                download: (fid) => callApi('get_group_file_url', { group_id: groupId, file_id: fid })
                    .then((res) => res?.data ?? {})
                    .catch(() => ({})),
                get root_files() {
    ...
    function createOneBotFriendAdapter(userId, userName) {
        ...
        return wrapCompatValue({
            ...
            getFileInfo: (fid) => callApi('_get_private_file_info', { user_id: userId, file_id: fid })
                .then((res) => res?.data ?? {})
                .catch(() => ({})),
            sendFile: (file, filename) => callApi('upload_private_file', { user_id: userId, file: String(file), name: filename ?? 'file' })
                .then((res) => res?.data?.file_id ?? '')
                .catch(() => ''),
            recallFile: (fid) => callApi('_recall_private_file', { user_id: userId, file_id: fid }).catch(() => false),
            forwardFile: (fid, groupId) => callApi('_forward_file', { user_id: userId, file_id: fid, group_id: groupId })
                .then((res) => res?.data?.file_id ?? '')
                .catch(() => ''),
            ...
            setFriendReq: (seq, yes = true, remark = '') => callApi('setFriendAddRequest', { flag: String(seq), approve: yes, remark }).catch(() => false),
            setGroupReq: (_gid, seq, yes = true, reason = '') => callApi('setGroupAddRequest', { flag: String(seq), approve: yes, reason, type: 'add' }).catch(() => false),
            setGroupInvite: (_gid, seq, yes = true) => callApi('setGroupAddRequest', { flag: String(seq), approve: yes, type: 'invite' }).catch(() => false),
    ...
    function createOneBotBotAdapter(state) {
        ...
        return {
            ...
            getGroupMemberInfo: (gid, uid) => callApi('getGroupMemberInfo', { group_id: gid, user_id: uid })
                .then((res) => normalizeMemberRecord(res?.data ?? {}))
                .catch(() => ({})),
