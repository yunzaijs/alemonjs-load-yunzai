	...
	*/
	function normalizeSegments(message) {
		return message.map((seg) => {
			if (!seg || typeof seg !== "object" || !seg.data || typeof seg.data !== "object" || Array.isArray(seg.data)) return seg;
			...
			if (type === "node" && data.content !== void 0) return {
				type,
	...
	function normalizeForwardMessages(result) {
		const payload = result?.data ?? result;
		return (Array.isArray(payload?.messages) ? payload.messages : Array.isArray(payload) ? payload : []).map((item) => {
	...
	function normalizeMessage(result) {
		if (result?.status === "failed" || result?.retcode !== void 0 && Number(result.retcode) !== 0) throw new Error(`getMsg 失败: ${result?.wording ?? result?.message ?? result.retcode}`);
		const payload = result?.data ?? result;
		if (!payload || typeof payload !== "object" || payload.message_id === void 0) throw new Error("getMsg 未返回有效消息");
	...
	function createOneBotGroupAdapter(groupId, opts) {
		return wrapCompatValue({
			...
			*/
			getMsg: (messageId) => callApi("getMsg", { message_id: messageId }).then(normalizeMessage),
	...
	function createOneBotBotAdapter(state) {
		...
		return {
			...
			getForwardMsg: (resId) => callApi("getForwardMsg", { id: resId }).then(normalizeForwardMessages).catch(() => []),
			restart: () => callApi("restartYunzai", {}, 5e3),
	...
	function buildOneBotEvent({ data, msgId, selfId, reply }) {
		...
		const rawSource = raw.source ?? (replySegment ? {
		...
		const source = rawSource && typeof rawSource === "object" && Array.isArray(rawSource.message) ? {
			...rawSource,
			message: normalizeSegments(rawSource.message)
		} : rawSource;
		if (isGroup && groupId) {
			cacheGroupRecord(groupId, { group_name: raw.group_name });
			cacheMemberRecord(groupId, userId, {
