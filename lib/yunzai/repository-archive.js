...
import { gitExecSync, hasNativeGit } from "./git.js";
...
//#region src/yunzai/repository-archive.ts
...
function assertArchiveTarget(value) {
	if (!isArchiveTarget(value)) throw new Error("未知压缩包类型");
}
...
function resolveArchiveRoot(extractDir, target) {
	if (archiveRootLooksValid(extractDir, target)) return extractDir;
	...
	if (entries.length === 1 && entries[0].isDirectory()) {
		...
		if (archiveRootLooksValid(child, target)) return child;
	}
...
function saveRepositoryArchive(targetValue, uploadedPath, originalName) {
	...
	if (hasExistingArchive) renameSync(destination, backup);
	try {
		renameSync(uploadedPath, destination);
	} catch (err) {
		if (hasExistingArchive && existsSync(backup)) renameSync(backup, destination);
		throw err;
	}
...
async function extractRepositoryArchive(targetValue) {
	assertArchiveTarget(targetValue);
	if (manager.isRunning) throw new Error("Yunzai 正在运行，请先停止后再解压压缩包");
	const sourceArchive = archivePath(targetValue);
	if (!existsSync(sourceArchive)) throw new Error("请先上传压缩包");
...
async function repairRepositoryArchiveOrigin(targetValue, repoUrl) {
	assertArchiveTarget(targetValue);
	if (manager.isRunning) throw new Error("Yunzai 正在运行，请先停止后再修复仓库来源");
	const targetDir = destinationPath(targetValue);
	if (!existsSync(targetDir)) throw new Error("请先解压对应压缩包");
	if (!repoUrl.trim()) throw new Error("请先填写 Git 仓库地址并保存");
	if (hasNativeGit()) {
