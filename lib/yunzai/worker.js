...
import { runWithExecutionContext, getExecutionContextForAction } from './execution-context.js';
...
function pickSegmentParams(msg, keys) {
    ...
    const params = Object.fromEntries(keys.map(key => [key, source?.[key]]).filter(([, value]) => typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean'));
