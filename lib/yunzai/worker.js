import { decodeCommandError, decodeCommandOutput, installWindowsCommandDecoding, repairWindowsCommandPath } from "./command-output.js";
...
import { installPluginFaultBoundary } from "./plugin-faults.js";
...
*/
function callApi(action, params = {}, timeout = 15e3, rawOneBot = false) {
	let context;
	try {
...
const compatibleConfigs = /* @__PURE__ */ new WeakSet();
/**
* 加载 Yunzai 目录中可用的 icqq/oicq 核心工具。
* jce/pb 等是纯数据编解码，可以安全复用；真正的 QQ SSO 连接仍不可从
* OneBot/qq-bot 适配器中伪造，因此 sendUni 会明确返回不支持错误。
*/
function loadIcqqModule() {
	try {
		return createRequire(path.join(process.cwd(), "package.json"))("icqq") ?? {};
	} catch {
		return {};
	}
}
function injectGlobals() {
	...
	Object.defineProperty(adapters, "push", {
		...
		writable: true,
		value(...items) {
			for (const item of items) if (!item?.path || !this.some((adapter) => adapter.path === item.path)) Array.prototype.push.call(this, item);
			return this.length;
		}
	});
	const botInstance = {
		...
		*/
		...
		sleepTimeout,
		sleep: (time, promise) => {
			const timeout = new Promise((resolve) => setTimeout(resolve, time, sleepTimeout));
			return promise ? Promise.race([promise, timeout]) : timeout;
		},
		/** icqq 底层 SSO 接口无法由 OneBot/qq-bot 适配器实现。 */
		sendUni: () => Promise.reject(/* @__PURE__ */ new Error("Bot.sendUni 在 AlemonJS 适配器中不可用，请改用 Bot.sendApi 或平台标准 API")),
		sig: { seq: 0 },
		...
		getTimeDiff: (time1 = botInstance.stat?.start_time * 1e3, time2 = Date.now()) => {
			...
			return `${days ? `${days}天` : ""}${hour ? `${hour}时` : ""}${min ? `${min}分` : ""}${sec ? `${sec}秒` : ""}${ms ? ms : ""}` || "0秒";
		},
		/**
		...
		*/
		exec: (command, options = {}) => new Promise((resolve) => {
			...
			const done = (error, stdout, stderr) => {
				...
				botInstance.makeLog(quiet ? "debug" : error ? "error" : "mark", `${displayName} [完成${botInstance.getTimeDiff(startedAt)}]${result.stdout ? `\n${result.stdout}` : ""}${result.stderr ? `\n${result.stderr}` : ""}`, "Command");
				resolve(result);
			};
			if (Array.isArray(command)) {
				const [file, ...args] = command;
				execFile(file, args, execOptions, done);
			} else exec(command, execOptions, done);
		}),
		...
		sendFriendMsg: (uid, msg) => botInstance.sendPrivateMsg(uid, msg),
		/**
		* TRSS/Yunzai 主人消息快捷入口。
		* 向当前 Yunzai 配置中的所有 masterQQ 发送，未配置主人时明确失败。
		*/
		sendMasterMsg: (msg) => {
			const masters = Array.isArray(globalThis._yunzaiCfg?.masterQQ) ? globalThis._yunzaiCfg.masterQQ : [];
			if (masters.length === 0) throw new Error("未配置 masterQQ，无法发送主人消息");
			return Promise.all(masters.map((uid) => botInstance.sendPrivateMsg(Number(uid), msg)));
		},
		/** TRSS/Yunzai 日志别名，统一转入 Worker 日志通道。 */
		makeLog: (level, ...args) => {
			return (typeof botInstance.logger?.[level] === "function" ? botInstance.logger[level] : botInstance.logger.info)(...args);
		},
		/**
		...
		*/
		fileToUrl: async (file) => {
			if (Buffer.isBuffer(file)) return `data:application/octet-stream;base64,${file.toString("base64")}`;
			const value = String(file ?? "");
			if (/^(https?:|data:|base64:|file:)/i.test(value)) return value;
			if (value.startsWith("/")) try {
				return `data:application/octet-stream;base64,${(await fs.promises.readFile(value)).toString("base64")}`;
			} catch {
				return value;
			}
			return value;
		...
		*/
		sendApi: (actionOrRequest, params = {}) => {
			const action = typeof actionOrRequest === "object" && actionOrRequest !== null ? String(actionOrRequest.action ?? "").trim() : String(actionOrRequest ?? "").trim();
			const requestParams = typeof actionOrRequest === "object" && actionOrRequest !== null ? actionOrRequest.params ?? {} : params;
			if (!action) return Promise.reject(/* @__PURE__ */ new Error("sendApi 缺少 action"));
			return callApi(action, requestParams, 15e3, true);
		},
		/** 获取群列表（填充 gl） */
		...
		*/
		...
		makeForwardArray: (msgs = [], node = {}) => {
			return buildForwardMsgCompat((Array.isArray(msgs) ? msgs : [msgs]).map((message) => ({
				...node,
				message
			})));
		},
	...
	botInstance.bots = new Proxy({}, {
		get(_target, prop) {
			if (prop === "get") return () => g.Bot ?? botInstance;
			if (prop === "has") return () => true;
			if (typeof prop === "string" && /^\d+$/.test(prop)) return g.Bot ?? botInstance;
		},
		has(_target, prop) {
...
*/
async function serializeForwardSendable(value) {
	if (Buffer.isBuffer(value)) return value.toString("base64");
	if (Array.isArray(value)) return Promise.all(value.map(serializeForwardSendable));
	...
	if ([
		...
		if (value.data && typeof value.data === "object" && !Array.isArray(value.data)) {
			...
			if (copy.data.file !== void 0) copy.data.file = await serializeReplyMediaFile(copy.data.file);
		} else if (copy.file !== void 0) copy.file = await serializeReplyMediaFile(copy.file);
	}
	return copy;
}
function serializeForwardNodes(nodes) {
	return Promise.all(nodes.map(async (node) => {
		const hasNodeData = node?.type === "node" && node?.data && typeof node.data === "object";
		const source = hasNodeData ? node.data : node;
		if (!source || typeof source !== "object") return node;
		const copy = { ...source };
		const key = source.content !== void 0 ? "content" : "message";
	...
	if (msg && typeof msg === "object") {
		...
		switch (msg.type) {
			...
			case "flash": {
				...
				const params = pickSegmentParams(msg, [
					"cache",
					"proxy",
					"timeout",
					"type",
			...
			case "markdown": {
				...
				return [{
					type: "markdown",
					data: typeof markdownData === "string" ? markdownData : JSON.stringify(markdownData),
					markdownData
				}];
			}
			case "raw": return [serializeNativeOnlySegment(msg)];
			case "xml": return [{
				type: "xml",
				data: msg.data ?? ""
			}];
...
function installConfigCompatibility(cfg) {
	if (!cfg || typeof cfg.getGroup !== "function" || compatibleConfigs.has(cfg)) return;
	try {
		const getGroup = cfg.getGroup.bind(cfg);
		cfg.getGroup = (first, second, ...rest) => getGroup(second === void 0 ? first : second, ...rest);
...
function buildEvent(data, msgId) {
	...
	if (isOneBotPlatform(data.platform) && raw && typeof raw === "object" && raw.post_type) return decorateEvent(oneBotRuntime.buildOneBotEvent({
		data,
		msgId,
		selfId,
		reply
	...
	if (eventName && !isMessageEventName(eventName)) return decorateEvent(buildFallbackNonMessageEvent(data, selfId, platformTag, reply, eventName), data);
...
async function main() {
	...
	if (repairedWindowsPath.length > 0) log("info", `已补全 Windows PATH: ${repairedWindowsPath.join(";")}`);
	injectGlobals();
