...
import { decodeCommandOutput, repairWindowsCommandPath } from "./command-output.js";
...
if (repairedWindowsPath.length > 0) logger.info(`[Yunzai] 已补全 Windows PATH: ${repairedWindowsPath.join(";")}`);
/**
* Worker 自身通常输出 UTF-8，但插件以 inherit/pipe 执行 cmd 时可能直接写入 GBK。
* 保留 Buffer 到换行符再解码，避免多字节字符刚好被流分片截断。
*/
function forwardWorkerOutput(stream, level, tag) {
	if (!stream) return;
	let pending = Buffer.alloc(0);
	const flush = (line) => {
		const text = decodeCommandOutput(line).replace(/\r$/, "");
		if (text) logger[level](`[Yunzai] [${tag}] ${text}`);
	};
...
var YunzaiManager = class {
	...
	*/
	...
	async install(repoUrl = getDefaultRepo()) {
		...
		try {
			...
			if (!cloned && existsSync(yunzaiDir)) try {
				rmSync(yunzaiDir, {
					recursive: true,
					force: true
				});
			...
			else if (cloned) logger.warn("[Yunzai] 源码已保留；下次启动会自动重试依赖同步");
			throw err;
		} finally {
			this.endTask();
		}
	...
	async start(options = {}) {
		this.beginTask("启动");
		try {
			if (options.syncDependencies !== false && this.isInstalled) await this.syncDependencies("启动前");
			await this.startInternal();
		} finally {
			this.endTask();
		}
	...
	async restart(beforeStop) {
		...
		try {
			...
			await this.stopInternal();
			this.throwIfCancelled();
			if (this.isInstalled) await this.syncDependencies("重启前");
			await this.startInternal();
		} finally {
			this.endTask();
		}
	...
	*/
	...
	sendConfirmed(msg) {
		...
		return new Promise((resolve, reject) => {
			if (!worker?.connected) {
				reject(/* @__PURE__ */ new Error("Worker IPC 未连接"));
				return;
			}
	...
	*/
	async syncPluginDependencies(reason) {
		try {
			await this.syncDependencies(reason);
			return true;
		} catch (err) {
			if (this.taskCancelled) throw err;
	...
	*/
	...
	pnpmInstall(cwd) {
		return new Promise((resolve, reject) => {
			...
			this.bootstrapManagedPnpm().then(runInstall).catch(reject);
		});
	}
	/** 安装插件到 plugins 目录 */
	async installPlugin(plugin) {
		...
		try {
			...
			if (!pluginInstalled && existsSync(pluginDir)) try {
				rmSync(pluginDir, {
					recursive: true,
					force: true
				});
	...
	async installPluginArchive(archivePath, options) {
		...
		try {
			...
			if (!pluginInstalled && pluginDir && existsSync(pluginDir)) try {
				rmSync(pluginDir, {
					recursive: true,
					force: true
				});
	...
	*/
	async extractPluginArchiveFromFile(archivePath, options) {
		...
		try {
			...
			try {
				if (replacementCommitted) rmSync(backupDir, {
					recursive: true,
					force: true
				});
	...
	execPackageAdd(cwd, spec, devFlag, isTrss) {
		...
		if (!isTrss) return run(process.execPath, [
			YARN_PATH,
			"add",
			spec,
			devFlag
		].filter(Boolean));
		return this.bootstrapManagedPnpm().then((pnpm) => run(process.execPath, [
			pnpm,
			"add",
			spec,
			devFlag
