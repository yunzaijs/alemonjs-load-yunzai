...
import { canUseGenericOneBotFallback, getNativeOneBotRequest, getNativeQuotedForwardRequests, getReplyMessageId, isUnsupportedOneBotActionError, normalizeOneBotMediaSource, sendNativeForward, summarizeNativeOneBotRequest } from "./forward.js";
...
function appendContentsToFormat(format, contents) {
	for (const c of contents) switch (c.type) {
		...
		case "video":
			...
			break;
		case "forward":
			if (c.fallback?.length) appendContentsToFormat(format, c.fallback);
