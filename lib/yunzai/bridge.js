...
* 也不会并发触发 Yunzai 运行时的共享 Puppeteer 渲染器。
*/
...
function appendContentsToFormat(format, contents, platform) {
	...
	for (const c of contents) switch (c.type) {
		case "text":
			if (textAsMarkdown) format.addMarkdownOriginal(c.data);
...
async function handleApiRequest(req, msgId) {
	...
	if (action === "restartYunzai" && !req.rawOneBot) {
		...
		try {
			...
			if (!accepted) manager.sendToWorker({
				type: "api_response",
				reqId,
				ok: false,
				error
...
//#endregion
export { bindApiRequestListener, bridge_default as default };