...
export async function executeYunzaiActionLocal(data: PrimitiveRecord) {
...
export async function installPluginArchiveLocal(filePath: string, options?: { dirName?: string; originalName?: string }) {
  const plugin = await manager.installPluginArchive(filePath, options);

  return { message: `${plugin.label} 安装完成` };
}
